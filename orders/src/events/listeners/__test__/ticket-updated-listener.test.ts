...
it("finds, updates, and saves a ticket", async () => {
	...
	expect(updatedTicket!.version as number).toEqual(data.version);

	const data2: TicketUpdatedEvent["data"] = {
		...data,
		version: ticket.get("version") + 2,
	};

	await listener.onMessage(data2, message);

	const updatedTicket2 = await Ticket.findById(data2.id);

	expect(updatedTicket2!.title).toEqual(data2.title);
	expect(updatedTicket2!.price).toEqual(data2.price);
	expect(updatedTicket2!.version as number).toEqual(data2.version);

	const data3: TicketUpdatedEvent["data"] = {
		...data,
		id: ticket.id,
		version: ticket.get("version") + 3,
		title: faker.random.words(3),
		price: parseFloat(faker.commerce.price(0, 200)),
		userId: "1234",
	};
	...
	const updatedTicket3 = await Ticket.findById(data3.id);

	expect(updatedTicket3!.title).toEqual(data3.title);
	expect(updatedTicket3!.price).toEqual(data3.price);
	expect(updatedTicket3!.version as number).toEqual(data3.version);
});
